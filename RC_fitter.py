...
class RedClump():
    ...
    def log_prior_nataf(self, theta):
        ...
        """
        EWRC, B, M_RC, sigma_RC = theta
        if (0< EWRC < 2 and 0 < B < 2 and 12 < M_RC < 17 and 0 < sigma_RC < 1):
            return 0.0
        return -np.inf
    ...
    def log_likelihood_nataf(self, theta, M):
        ...
        N_obs = len(self.cmd.fitStarDict['altmag'])
        self.A = N_obs/self.integrator(EWRC, B, M_RC, sigma_RC, n=100)

        model = self.model_MCMC_nataf(theta, M)

        return np.sum(np.log(model))-N_obs
    ...
    def prepare_data_hist(self, M):
        ...
        Nstars, hist_bins = np.histogram(M,bins)#'rice')
...
if __name__ == "__main__":
    ...
    rc_fitter = RedClump(cmd, binnumber=int(.05*len(cmd.fitStarDict['altmag'])))
    M_dumb, N, Nerr = rc_fitter.prepare_data_hist(rc_fitter.cmd.fitStarDict['altmag'])
    M = rc_fitter.cmd.fitStarDict['altmag']
    sampler, pos, prob, state = rc_fitter.run_MCMC(M)
    tstop = time.time()
    ...
    best_model = rc_fitter.model_MCMC_nataf(samples[best_ind], M_dumb)
    plt.plot(M_dumb, best_model, color="k", lw=2, alpha=0.8)
    ...
    labels = ['A','B',r'$M_{RC}$',r'$\sigma_{RC}$',r'$N_{RC}$']
    plt.clf()
